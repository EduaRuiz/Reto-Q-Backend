...
describe('UserMongoRepository', () => {
  ...
  describe('findAll', () => {
    ...
    it('should throw a MongoServerError when there is a conflict', (done) => {
      // Arrange
      const message = 'Conflict while getting users list';
      const error = new MongoServerError(new Error(message));
      jest.spyOn(model, 'find').mockReturnValueOnce({
  ...
  describe('findOneByEmail', () => {
    it('should return a User', (done) => {
      // Arrange
      const entityEmail = '123';
      const entity = {} as unknown as UserMongoModel;
      jest.spyOn(model, 'findOne').mockReturnValueOnce(of(entity) as any);

      // Act
      const result$ = repository.findOneByEmail(entityEmail);

      // Assert
      result$.subscribe({
        next: (result) => {
          expect(result).toBe(entity);
          expect(model.findOne).toHaveBeenCalledWith({ email: entityEmail });
          done();
        },
      });
    });
    ...
    it('should throw a MongoServerError when there is a conflict', (done) => {
      // Arrange
      const entityEmail = '123';
      const message = 'Conflict while getting user by email';
      const error = new MongoServerError(new Error(message));
      jest
        .spyOn(model, 'findOne')
        .mockReturnValueOnce(throwError(() => error) as any);
